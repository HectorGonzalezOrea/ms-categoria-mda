	...
	public ResponseEntity<?> actualizarPosicionArchivoPUT(
		...
		if (accept != null && accept.contains(HEADER_APP_JSON)) {
			try {
				...
				if (modificarPosicionReq == null || idArchivo == null || idArchivo.equals(Constantes.BLANK_SPACE)
						|| usuario == null || usuario.equals(Constantes.BLANK_SPACE) || origen == null
						|| origen.equals(Constantes.BLANK_SPACE) || destino == null || destino.equals(Constantes.BLANK_SPACE)) {
					
					BadRequest br = new BadRequest();
					br.setCodigo(Constantes.ERROR_CODE);
					br.setMensaje(Constantes.ERROR_MENSAJE);
				...
				if(modificarPosicionReq.getIdPrioridad() <1 ) {
					BadRequest br = new BadRequest();
					br.setCodigo(Constantes.ERROR_CODE);
					br.setMensaje(Constantes.ERROR_CONSOLIDACION);
					return new ResponseEntity<BadRequest>(br, HttpStatus.BAD_REQUEST);
				}
				if(idArchivo.equals(modificarPosicionReq.getIdArchivo().toString())) {
					...
					if (resp != null) {
						...
						isr.setCodigo(Constantes.ERROR_SERVER);
						isr.setMensaje(Constantes.ERROR_SERVER_MSG);
					...
					BadRequest br = new BadRequest();
					br.setCodigo(Constantes.ERROR_CODE);
					br.setMensaje(Constantes.ERROR_MENSAJE_IDS);
    ...
    public ResponseEntity<?> consultaConsolidadosArchivosGET(
		...
		if (accept != null && accept.contains(HEADER_APP_JSON)) {
			...
			if (usuario == null || origen == null || destino == null || vigencia == null) {
				...
				badReq.setCodigo(Constantes.ERROR_CODE);
				badReq.setMensaje(Constantes.ERROR_MENSAJE);
				log.info("{}", badReq);
			...
			if(!consolidadoService.validaFormatoFecha(vigencia)){
				...
				badReq = new BadRequest();
				badReq.setCodigo(Constantes.ERROR_CODE);
				badReq.setMensaje(Constantes.ERROR_MENSAJE_DATE);
				return new ResponseEntity<BadRequest>(badReq, HttpStatus.BAD_REQUEST);
			}
			List<ConsultarArchivoConsolidadoResInner> result = consolidadoService.getConsolidados(vigencia);
			...
			if (!result.isEmpty()) {
				...
				nf.setCodigo(Constantes.CODE_NOT_FOUND);
				nf.setMensaje(Constantes.MESSAGE_NOT_FOUND);
            ...
            try {
            	...
            	if(idArchivo == null || idArchivo.equals("") || usuario == null || usuario.equals("") || origen == null || origen.equals("")  || destino == null || destino.equals("") ) {
             	...
             	if(Boolean.TRUE.equals(eliminado)) {
             		sr.setCodigo(Constantes.EXITO_ELIMINAR);
             		sr.setMensaje(Constantes.EXITO_ELIMINAR_MSG);
             		...
             		sr.setCodigo(Constantes.ERROR_SERVER);
             		sr.setMensaje(Constantes.NO_EXITO_ELIMINAR_MSG);    		
        ...
        if (accept != null && accept.contains(HEADER_APP_JSON)) {
        	try {
        		...
        		if(usuario != null && origen != null && destino != null && fechaAplicacion != null) {
            		...
            		sr.setCodigo(Constantes.EXITO_PROCESAR);
            		sr.setMensaje(Constantes.EXITO_PROCESAR_MSG);
	...
	public ResponseEntity<?> registrarConsolidadoPOST(
		...
		if (accept != null && accept.contains(HEADER_APP_JSON)) {
			...
			if (usuario == null || origen == null || destino == null || adjunto == null || vigencia == null
				...
				bad.setMensaje(Constantes.ERROR_MENSAJE);
				log.info("{}", bad);
			...
			if(Boolean.FALSE.equals(adjunto.getOriginalFilename().toLowerCase().endsWith(CSV_MIN))) {
				bad = new BadRequest();
				bad.setCodigo(Constantes.ERROR_CODE);
				bad.setMensaje(Constantes.ERROR_BAD_REQUEST_EXT);
				log.info("{}", bad);
				return new ResponseEntity<BadRequest>(bad, HttpStatus.BAD_REQUEST);
			}
			try {
				...
				if (service) {
					...
					res.setMessage(Constantes.EXITO_GUARDAR);
					log.info("{}", res);
