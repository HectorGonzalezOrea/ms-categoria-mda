...
public class UsuariosApiController implements UsuariosApi {
	...
	public ResponseEntity<?> historialUsuarioPOST(
	  ...
	  public ResponseEntity<?> crearUsuarioPOST(
			  @ApiParam(value = "Usuario en el sistema origen que lanza la petición." ,required=true) @RequestHeader(value="usuario", required=true) String usuario,
			  @ApiParam(value = "Sistema que origina la petición." ,required=true, allowableValues="portalMotorDescuentosAutomatizados") @RequestHeader(value="origen", required=true) String origen,
			  @ApiParam(value = "Destino final de la información." ,required=true, allowableValues="Mongo, mockserver") @RequestHeader(value="destino", required=true) String destino,
			  @ApiParam(value = "peticion para crear el usuario en el portal."  )  @Valid @RequestBody InfoUsuario peticion) {
	        String accept = request.getHeader("Accept");
	        if (accept != null && accept.contains("application/json")) {
	        	log.info("Empezamos");
	            try {
	            	...
	            	if (peticion != null) {
						...
						log.info("idUsuario: " + peticion.getIdUsuario());
						log.info("nombre: " + peticion.getNombre());
						log.info("apellidoPaterno: " + peticion.getApellidoPaterno());
						log.info("apellidoMaterno: " + peticion.getApellidoMaterno());
						log.info("usuario: " + peticion.getUsuario());
						log.info("puesto: " + peticion.getPuesto());
						log.info("direcion: " + peticion.getDireccion());
						
						
						InfoUsuario resp = usuarioService.crearUsuario(peticion.getIdUsuario(), peticion.getNombre(), peticion.getApellidoPaterno(), peticion.getApellidoMaterno(), peticion.getUsuario(), peticion.getPuesto(),
								peticion.getDireccion(), peticion.getSubdireccion(), peticion.getGerencia(), peticion.getDepartamentoArea(), peticion.getPerfil(), peticion.isActivo());
						
						if(resp != null) {
							return new ResponseEntity<InfoUsuario>(resp, HttpStatus.OK);
						} else {
							InternalServerError ie = new InternalServerError();
							ie.setCodigo("NMP-MDA-500");
							ie.setMensaje("Error interno del servidor");
